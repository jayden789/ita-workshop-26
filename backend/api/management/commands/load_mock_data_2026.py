...
def setup_alice_regn(alice, workshop, dates, regn_options):
    ...
    regn.attending_dates.set([dates[d] for d in range(8, 14)])
    regn.options.set(
        [
            regn_options["ita26_sundayReception_notAttending"],
            regn_options["ita26_banquetSelf_selfOnly"],
            regn_options["ita26_valentinesEvent_selfOnly"],
        ]
    )
    regn.save()
    return regn
...
def setup_bob_regn(bob, workshop, dates, regn_options):
    ...
    regn.attending_dates.set([dates[d] for d in [9, 10, 12]])
    regn.options.set(
        [
            regn_options["ita26_sundayReception_selfPlus2"],
            regn_options["ita26_banquetSelf_selfPlus1"],
            regn_options["ita26_valentinesEvent_selfPlus1"],
        ]
    )
...
def setup_carol_regn(carol, workshop, dates, regn_options):
    ...
    regn.attending_dates.set([dates[11], dates[13]])
    regn.options.set(
        [
            regn_options["ita26_banquetSelf_notAttending"],
            regn_options["ita26_valentinesEvent_selfPlus2"],
        ]
    )
    regn.save()
    return regn
...
def _random_sunday_reception_options():
    if random.random() < 0.3:
        if random.random() < 0.5:
            return []
        return ["ita26_sundayReception_notAttending"]
...
def _random_banquet_self_options():
    if random.random() < 0.2:
        if random.random() < 0.5:
            return []
        return ["ita26_banquetSelf_notAttending"]
    return random.choices(
        [
...
def _random_monday_lunch_options():
    if random.random() < 0.3:
        return []
    return random.choices(
        ["ita26_mondayLunch_attending", "ita26_mondayLunch_notAttending"]
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):  # pylint: disable=too-many-locals
        ...
        ita26 = Workshop.objects.get(slug="ita26")
        dates = {
            day: Date.objects.get(date_value=datetime.date(2026, 2, day))
            for day in range(8, 14)
        }
        regn_options = {
            option.slug: option for option in ita26.registration_options.all()
        }
        ...
        with transaction.atomic():
            ...
            ita26.save()
            self.print_success(
                "Set {} open for registration".format(ita26.slug)
            )
            ...
            alice_regn = setup_alice_regn(
                users["alice"], ita26, dates, regn_options
            )
            bob_regn = setup_bob_regn(users["bob"], ita26, dates, regn_options)
            carol_regn = setup_carol_regn(
                users["carol"], ita26, dates, regn_options
            )
            ...
            random_regns = setup_random_registrants(
                ita26, dates, regn_options, [users["ivy"], users["julian"]]
            )
